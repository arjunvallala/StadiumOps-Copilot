 ...
 * 
 * @param {'pre-match' | 'live' | 'halftime' | 'post-match'} phase 
 * @param {Array<{ id: string, name: string, occupancy: number, status: string }>} gateAdvisories 
 * @returns {{
 *   phase: string,
 *   summary: string,
 *   suggestions: Array<{ location: string, allocationPercentage: number, task: string, reasoning: string }>
 * }}
 */
export function getShiftSuggestions(phase, gateAdvisories = []) {
  const normalizedPhase = (phase || 'pre-match').toLowerCase();
  
  // Find critical and warning gates
  const criticalGates = gateAdvisories.filter(g => g.status === 'critical');
  const warningGates = gateAdvisories.filter(g => g.status === 'warning');
  ...
  switch (normalizedPhase) {
    case 'pre-match':
      summary = 'Inbound flow phase: Focus heavily on gate entry scanning, ticket validation, and queue sorting.';
      ...
      // Dynamic adjustment if gates are critical/warning
      if (criticalGates.length > 0) {
        // Shift percentage to critical gates
        const criticalGateNames = criticalGates.map(g => g.name).join(', ');
        suggestions = suggestions.map(s => {
          if (s.location === 'Concourse Information Desks') {
            return { ...s, allocationPercentage: 10 }; // reduce helper staff
          }
          if (s.location === 'Main Entry Gates') {
            return {
              ...
              reasoning: `DANGER: Critical overcrowding detected at ${criticalGateNames} (${criticalGates[0].occupancy}%). 10% staff reallocated here to manage crowd diversion and prevent gate blockages.`
            };
          }
          return s;
        });
      } else if (warningGates.length > 0) {
        const warningGateNames = warningGates.map(g => g.name).join(', ');
        suggestions = suggestions.map(s => {
          if (s.location === 'Main Entry Gates') {
            return {
              ...
              reasoning: `Queue backup detected at ${warningGateNames} (${warningGates[0].occupancy}%). Deploying extra support to clear lines.`
            };
          }
          if (s.location === 'Concourse Information Desks') {
            return { ...s, allocationPercentage: 15 };
          }
          return s;
        });
      }
    ...
    case 'post-match':
      summary = 'Outbound flow phase: Spectators exiting stadium. Focus on exit gates, public transit terminals, and perimeter safety.';
      suggestions = [
        {
      ...
      // Reallocate if there is exit bottlenecking
      if (criticalGates.length > 0 || warningGates.length > 0) {
        const targetGates = criticalGates.length > 0 ? criticalGates : warningGates;
        const gateNames = targetGates.map(g => g.name).join(', ');
        suggestions = suggestions.map(s => {
          if (s.location === 'Exit Gates & Exit Paths') {
            return {
              ...
              reasoning: `Bottlenecks detected at egress points: ${gateNames}. Reallocated 10% staff to guide exiting spectators smoothly and prevent crowd build-up.`
            };
          }
          if (s.location === 'Lost & Found / Info Points') {
            return { ...s, allocationPercentage: 0 }; // close lost/found, redirect all to exits
          }
          if (s.location === 'Transit Terminals (Bus/Train)') {
            return { ...s, allocationPercentage: 40 };
          }
          return s;
        });
      }
